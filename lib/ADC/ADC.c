...
void adc_init() {
    ...
    for (volatile int i = 0; i < 1000; i++);//puede ser delay_ms(1); -> 1,17u"
...
unsigned int adc_read(int canal, int vMax) {
    ...
    ADC1->SQR3 |= (canal & 0x1F); // ESCRIBE EN SQ1 (le indica la direccion del registro que quiero leer)
    ...
    // SR -> STATUS REGISTER -> siempre que la conversion se este dando bien, vale 1. Indica si el proceso se esta dando bien
    // EOC -> END OF CONVERTION ; CUANDO TERMINA LA CONVERSION SE PONE EN 1
    while (!(ADC1->SR & ADC_SR_EOC)); //es una espera que dura hasta que termine la conversión
    ...
    //porque el valor de bits que da es impredecible, entoces lo pasamos a volts, y eso lo ponderamos a 0 o 1
